...
class Pick_and_Place:
    ...
    def predict_position_dynamic(self, w_t, q_current):
    
        len_block_world, block_world = self.get_block_world(q_current)
        
       
    
        world_in_table = np.array(([1, 0, 0, 0 ],
                                       [0,1, 0, -0.990 ],
                                       [0, 0,1, -0.2 ],
                                       [0, 0, 0, 1   ]))
    
        cur_time = time_in_seconds()
        while len_block_world == 0:
            run_time = time_in_seconds() - cur_time
            if (run_time)>20:
                break
            elif run_time%2 == 0:
                len_block_world, block_world = self.get_block_world(q_current) 
        ...
        block_table = world_in_table @ block_world[0]
        ...
        r = np.sqrt(x**2 + y**2)
        theta_initial = np.arctan2(y, x)
    
   
        angular_displacement = w_t
    
    
        theta_final = theta_initial + angular_displacement
   
        new_x = r * np.cos(theta_final)
        new_y = r * np.sin(theta_final)
    	
        block_table[0,3] = new_x
        block_table[1,3] = new_y
    	
    	
        table_in_world = np.array(([1, 0, 0, 0 ],
                                       [0,1, 0, 0.990 ],
                                       [0, 0,1, 0.2 ],
                                       [0, 0, 0, 1   ]))
    	
        shifted_block_world = table_in_world @ block_table
    	
        return shifted_block_world


    def move_to_dynamic_pick(self, q_current, w_t):
        ...
        q_above_rotate = self.calculate_q_via_ik(pos_above_rotate, q_current)
        arm.safe_move_to_position(q_above_rotate)
        
        shifted_block_world = self.predict_position_dynamic(w_t, q_current)
        ...
        ee_rot = np.array(([1,0,0],
	    			...
	    			[0,0,-1],
	    			[0,0,0]))
        block_pos = shifted_block_world[:,3]
        block_pos = block_pos.reshape(4,1)
        ee_goal = np.hstack((ee_rot,block_pos))
	
        q_dynamic_block = self.calculate_q_via_ik(ee_goal, q_above_rotate)
            
        self.drop_block()
        arm.safe_move_to_position(q_dynamic_block)
        self.grab_block()
        arm.safe_move_to_position(self.q_above_drop)
...
if __name__ == "__main__":
    ...
    pick_and_place.move_to_dynamic_pick(start_position, 5)
