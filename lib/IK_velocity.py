...
def IK_velocity(q_in, v_in, omega_in):
    ...
    v_desired =  np.vstack((v_in, omega_in))

    J = calcJacobian(q_in)

    # Handle NaN values
    nanValues = np.isnan(v_desired)
    J = J[~nanValues.flatten()]
    v_desired = v_desired[~nanValues]

    dq, residuals, rank, s = np.linalg.lstsq(J, v_desired, rcond=None)
